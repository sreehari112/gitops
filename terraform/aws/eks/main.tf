...
module "eks" {
  ...
  aws_auth_roles = [
    ...
    {
      rolearn  = "arn:aws:iam::637423629731:role/KubernetesAdmin"
      username = "arn:aws:iam::637423629731:role/KubernetesAdmin"
      groups   = ["system:masters"]
    },
    {
      rolearn  = "arn:aws:iam::637423629731:role/argocd-managementcluster"
      username = "arn:aws:iam::637423629731:role/argocd-managementcluster"
      groups   = ["system:masters"]
    },
    {
      rolearn  = "arn:aws:iam::637423629731:role/atlantis-managementcluster"
      username = "arn:aws:iam::637423629731:role/atlantis-managementcluster"
      groups   = ["system:masters"]
    },
  ]
  ...
  eks_managed_node_groups = {
    ...
    default_node_group = {
      desired_size = tonumber("3") # tonumber() is used for a string token value
      min_size     = tonumber("3") # tonumber() is used for a string token value
      max_size     = tonumber("3") # tonumber() is used for a string token value
...
module "vpc_cni_irsa" {
  ...
  role_name             = upper("VPC-CNI-IRSA-managementcluster")
...
module "aws_ebs_csi_driver" {
  ...
  role_name = upper("EBS-CSI-DRIVER-managementcluster")
...
data "aws_iam_policy_document" "crossplane_custom_trust_policy" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${split("arn:aws:iam::637423629731:oidc-provider/", module.eks.oidc_provider_arn)[1]}:aud"
      values   = ["sts.amazonaws.com"]
    }

    condition {
      test     = "StringLike"
      variable = "${split("arn:aws:iam::637423629731:oidc-provider/", module.eks.oidc_provider_arn)[1]}:sub"
      values   = ["system:serviceaccount:crossplane-system:crossplane-provider-terraform-managementcluster"]
    }
  ...
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::637423629731:role/KubernetesAdmin"]
    }
  }
  statement {
    effect  = "Allow"
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::637423629731:role/argocd-${local.name}"]
    }
  }
}
